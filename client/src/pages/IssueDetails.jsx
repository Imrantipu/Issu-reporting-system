...
const IssueDetails = () => {
  ...
  const boostMutation = useMutation({
    ...
    onSuccess: () => {
      setFeedback('Boost intent created. Complete payment to apply priority.');
      queryClient.invalidateQueries({ queryKey: ['issue', id] });
      queryClient.invalidateQueries({ queryKey: ['issues'] });
    },
    onError: (err) => {
  ...
  const handleDelete = () => {
    setFeedback('');
    if (!isOwner) {
      setFeedback('Only the reporter can delete this issue.');
      return;
    }
  ...
  const handleEdit = () => {
    setFeedback('Edit flow/modal goes here (only pending issues).');
  };
